    ...
    try:
        for index, entry in enumerate(dialogue["dialogue"]):  # Use enumerate to get the index
            ...
            if hasattr(response, 'content') and isinstance(response.content, bytes):
                # Save the audio to a file in Streamlit's temp directory
                audio_file_path = os.path.join(st.runtime.get_instance().temp_dir, f"{role.lower()}_output_{index}.mp3")  # Include index in filename
        ...
        # Save the stitched audio to a single file in Streamlit's temp directory
